...
public class ScoreBoards {
    ...
    private void formatScoreboard(Objective objective, Scoreboard board, String team, ChatColor color) { //TODO !!! fix this problem with scoreboards https://gyazo.com/ac41d92082364447927f3e3c8ff0918d
        ...
        if (team.equals("hunters")) for (boolean booleanValue : booleans)
            generateState(board, team, booleanValue);
        else for (int index = booleans.length - 1; index >= 0; index--)
            generateState(board, team, booleans[index]);
        objective.getScore(team).setScore(scoreIndex);
    }

    private void generateState(Scoreboard board, String team, boolean hunters) { // state "\\lives and etc. or if player is dead"
        if (hunters) {
            for (Hunter hunter : ManHunt.getInstance().getHunters()) {
                returnTeam(board, team, hunter.getPlayerId()).setPrefix(ChatColor.LIGHT_PURPLE + Bukkit.getPlayer(hunter.getPlayerId()).getName() + " " + ChatColor.GRAY + ChatColor.UNDERLINE + hunter.getLives());
                Bukkit.getLogger().info(String.valueOf(scoreIndex));
                scoreIndex--;
            }
        } else {
            for (UUID uuid : ManHunt.getInstance().getRunners()) {
                returnTeam(board, team, uuid).setPrefix(ChatColor.LIGHT_PURPLE + Bukkit.getPlayer(uuid).getName() + ChatColor.GRAY + ChatColor.UNDERLINE + " Alive");
                Bukkit.getLogger().info(String.valueOf(scoreIndex));
                scoreIndex--;
            }
        }
    ...
    private Team returnTeam(Scoreboard board, String team, UUID uuid) {
        Team gameTeam = board.registerNewTeam(team + ":" + uuid); gameTeam.addEntry(team);
        return gameTeam;
    }
    ...
    public void removeScoreBoards() { //TODO scoreboards don't remove !!!
        ManHunt.getInstance().getMerged().forEach(player -> Bukkit.getPlayer(player).setScoreboard(Bukkit.getScoreboardManager().getNewScoreboard()));
    }
    ...
    public void updateScoreBoards(Player player) { // state "\\lives and etc. or if player is dead"
        ...
        for (Map.Entry<String, Scoreboard> scoreboard : scoreboards.entrySet()) {
            if (ManHunt.getInstance().returnFilteredHunters().contains(uuid)) {
                ...
                scoreboard.getValue().getTeam("hunters" + uuid).setPrefix(ChatColor.LIGHT_PURPLE.toString() + Bukkit.getPlayer(ManHunt.getInstance().returnHunterObject(uuid).getPlayerId()) + " " + ChatColor.GRAY + ChatColor.UNDERLINE + (lives == 0 ? "Dead" : lives));
            } else if (ManHunt.getInstance().getRunners().contains(uuid)) {
                scoreboard.getValue().getTeam("runners" + uuid).setPrefix(ChatColor.LIGHT_PURPLE.toString() + Bukkit.getPlayer(uuid) + " " + ChatColor.GRAY + ChatColor.UNDERLINE + "Dead");
            }
        }
    }
}
